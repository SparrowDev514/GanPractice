...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        rotate_button_layout = QtWidgets.QVBoxLayout()
        ...
        #回転選択ボタン
        image_rotate_t = QtWidgets.QRadioButton('回転あり')
        image_rotate_f = QtWidgets.QRadioButton('回転なし')

        #回転選択ボタングループ
        self.rotate_group = QtWidgets.QButtonGroup()
        self.rotate_group.addButton(image_rotate_t, 1)
        self.rotate_group.addButton(image_rotate_f, 0)
        self.rotate_group.button(1).setChecked(True)
        ...
        rotate_button_layout.addWidget(image_rotate_t)
        rotate_button_layout.addWidget(image_rotate_f)
        rotate_button_layout.addStretch()

        #設定レイアウト
        setting_layout.addLayout(bw_button_layout)
        setting_layout.addLayout(npy_button_layout)
        setting_layout.addLayout(rotate_button_layout)
    ...
    def on_start_btn(self):
        ...
        if(self.input_folder_path != '') and (self.output_folder_path != ''):
            ...
            is_rotate = bool(self.rotate_group.checkedId())
            try:
                target_size = int(self.target_size_form.text())
            except ValueError:
                print('画像のサイズは正の整数にしてください')
            result_image_list = WaterDown.process(input_image_list, is_black, is_rotate, target_size)
            # 処理した画像の保存
